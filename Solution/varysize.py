...
def dumpCSV(iters, times,rewards,steps,world,method,ow):
    ...
    if method == "Value":
        fname = 'Vary/VI/size {}:{}.csv'.format(ow,method)
    if method == "Policy":
        fname = 'Vary/PI/size {}:{}.csv'.format(ow,method)
    if method[:10] == "Q-Learning":
        fname = 'Vary/Q/size {}:{}.csv'.format(ow,method)
...
if __name__ == '__main__':
    if not os.path.exists("Vary/VI"):
        os.makedirs("Vary/VI")
    if not os.path.exists("Vary/PI"):
        os.makedirs("Vary/PI")
    if not os.path.exists("Vary/Q"):
        os.makedirs("Vary/Q")
    discount=0.99
    MAX_ITERATIONS = 1000;
    NUM_INTERVALS = 1000;
